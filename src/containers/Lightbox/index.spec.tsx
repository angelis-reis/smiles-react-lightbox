...
// 6. should be able to check the opt-out checkbox, click on close button and save a "optedOut" cookie in localStorage, OK
// 8. should not be able to render modal on page reload after accepted cookies OK
...
describe('Lightbox', () => {
	...
	it(' should be able renders correctly', async () => {
		await act(async () => render(<Lightbox />));
		const confirmButton = document.querySelector(
			'.modal-confirm-button'
		) as HTMLElement;
	...
	it(' should be able to close modal by clicking on close button', async () => {
		...
		act(() => userEvent.click(closeModalButton));
	...
	it(' should be able to close modal by clicking outside of modal', async () => {
		...
		act(() => userEvent.click(outsideModal));
		await waitForElementToBeRemoved(() =>
			screen.getByText(
				/ganhe ainda mais milhas ao transferir os pontos do seu cartão de crédito pra smiles e aproveite um mundo de oportunidades!/i
			)
	...
	it(' should be able to check opt-out-checkbox, close modal by clicking on close button, close modal and save a "optedOut" cookie in localStorage ', async () => {
		...
		act(() => userEvent.click(optOutCheckBox));
	...
	it(' should not be able to open modal on page reload after save a "optedOut" cookie in localStorage ', async () => {
